...
ProbeResult ProbeAppMemoryPressure() {
    ...
    const std::uint64_t expected_limit = MemoryManager::ExpectedAppMemoryUsageLimit();
    ...
    const std::uint64_t headroom = limit > usage_before ? limit - usage_before : 0;
    ...
    details << "limit=" << limit << ";expected_limit=" << expected_limit
            ...
            << ";peak_usage=" << peak_usage << ";usage_after=" << usage_after
    ...
    const DWORD result_error =
        fully_committed ? (accounting_observed ? ERROR_SUCCESS : ERROR_INVALID_DATA)
                        : allocation_error;
    return {"memory-pressure", fully_committed && accounting_observed, result_error,
            details.str()};
}
